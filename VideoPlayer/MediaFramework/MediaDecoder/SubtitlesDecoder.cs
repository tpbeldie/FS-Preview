...
namespace FSPreview.MediaFramework.MediaDecoder
{
    public unsafe class SubtitlesDecoder : DecoderBase
    {
        ...
        public ConcurrentQueue<SubtitlesFrame> Frames { get; protected set; } = new ConcurrentQueue<SubtitlesFrame>();
        ...
        protected override void RunInternal() {
            ...
            do {
                ...
                if (m_demuxer.SubtitlesPackets.Count == 0) {
                    CriticalArea = true;
                    lock (lockStatus) {
                        if (Status == Status.Running) {
                    ...
                    while (m_demuxer.SubtitlesPackets.Count == 0 && Status == Status.QueueEmpty) {
                ...
                lock (m_lockCodecCtx) {
                    if (Status == Status.Stopped || m_demuxer.SubtitlesPackets.Count == 0) {
                        continue;
                    }
                    m_demuxer.SubtitlesPackets.TryDequeue(out IntPtr pktPtr);
                    packet = (AVPacket*)pktPtr;
                    int gotFrame = 0;
                    AVSubtitle sub = new AVSubtitle();
                    ret = avcodec_decode_subtitle2(m_codecCtx, &sub, &gotFrame, packet);
                    if (ret < 0) {
                        allowedErrors--;
        ...
        private SubtitlesFrame ProcessSubtitlesFrame(AVPacket* packet, AVSubtitle* sub) {
            ...
            mFrame.timestamp = ((long)(packet->pts * SubtitlesStream.Timebase) - m_demuxer.StartTime) + Config.Subtitles.Delay;
            try {
                string line = "";
                byte[] buffer;
        ...
        public static string SSAtoSubStyles(string s, out List<SubStyle> styles) {
            ...
            if (bold.from != -1) {
                bold.len = soutPostLast - bold.from;
                styles.Add(bold);
            }
            if (italic.from != -1) {
                italic.len = soutPostLast - italic.from;
                styles.Add(italic);
            }
            if (strikeout.from != -1) {
                strikeout.len = soutPostLast - strikeout.from;
                styles.Add(strikeout);
            }
            if (underline.from != -1) {
                underline.len = soutPostLast - underline.from;
                styles.Add(underline);
            }
            if (color.from != -1 && color.value != Color.Transparent) {
                color.len = soutPostLast - color.from;
                styles.Add(color);
            }
            return sout;
        }
