...
namespace FSPreview.MediaFramework
{
    public abstract class RunThreadBase : NotifyPropertyChanged
    {
        ...
        public Status Status {
            ...
            set {
                lock (lockStatus) {
#if DEBUG
                    if (m_status != Status.QueueFull && value != Status.QueueFull && m_status != Status.QueueEmpty && value != Status.QueueEmpty) Log($"{m_status} -> {value}");
#endif
                    m_status = value;
                }
            }
        ...
        internal void Log(string msg) { 
            Debug.WriteLine($"[{DateTime.Now.ToString("hh.mm.ss.fff")}] [#{UniqueId}] [{threadName}] {msg}"); 
        }
