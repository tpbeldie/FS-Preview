...
namespace FSPreview.MediaFramework.MediaContext
{
    ...
    {
        /// <summary>
        /// The backend m_demuxer. Access this to enable the required streams for downloading
        /// </summary>
        ...
        /// </summary>
        public long CurTime { get => m_CurTime; private set => Set(ref m_CurTime, value); }
        long m_CurTime;

        /// <summary>
        /// The total duration of the input (Ticks)
        /// </summary>
        public long Duration { get => m_Duration; private set => Set(ref m_Duration, value); }
        long m_Duration;

        /// <summary>
        /// The percentage of the current download process (0 for live streams)
        /// </summary>
        public double DownloadPercentage { get => m_DownloadPercentage; set => Set(ref m_DownloadPercentage, value); }
        double m_DownloadPercentage;
        double m_downPercentageFactor;

        public Downloader(Config config, int uniqueId = -1) : base(uniqueId) {
            DecCtx = new DecoderContext(config, null, UniqueId, false);
            DecCtx.AudioDemuxer.Config = config.Demuxer.Clone(); // We change buffer duration and we need to avoid changing video m_demuxer's config
        ...
        public string Open(string url, bool defaultInput = true, bool defaultVideo = true, bool defaultAudio = true) {
            lock (lockActions) {
                ...
                m_downPercentageFactor = Duration / 100.0;
                return null;
            }
        }
        ...
        /// </summary>
        /// <param name="filename">The filename for the downloaded video. The file extension will let the m_demuxer to choose the output format (eg. mp4). If you useRecommendedExtension will be updated with the extension.</param>
        ...
        protected override void RunInternal() {
            ...
            do {
                ...
                if (curDT - secondTicks > 1000 * 10000 && (!isAudioDemuxer || DecCtx.VideoDemuxer.Disposed)) {
                    ...
                    if (m_Duration > 0) {
                        DownloadPercentage = CurTime / m_downPercentageFactor;
                    }
                }
