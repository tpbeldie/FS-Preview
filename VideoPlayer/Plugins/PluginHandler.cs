...
namespace FSPreview.Plugins
{
    public class PluginHandler
    {
        ...
        public Dictionary<string, IProvideAudio> PluginsProvideAudio { get; private set; }
       
        public Dictionary<string, IProvideVideo> PluginsProvideVideo { get; private set; }
      
        public Dictionary<string, IProvideSubtitles> PluginsProvideSubtitles { get; private set; }

        public Dictionary<string, ISuggestAudioInput> PluginsSuggestAudioInput { get; private set; }
       
        public Dictionary<string, ISuggestVideoInput> PluginsSuggestVideoInput { get; private set; }
       
        public Dictionary<string, ISuggestSubtitlesInput> PluginsSuggestSubtitlesInput { get; private set; }
        ...
        public Dictionary<string, ISearchSubtitles> PluginsSearchSubtitles { get; private set; }
                ...
                foreach (string dir in dirs) {
                    foreach (string file in Directory.GetFiles(dir, "*.dll")) {
                        try {
                            ...
                            Utils.Log($"[PluginHandler] [Error] Failed to load assembly ({e.Message} {Utils.GetRecInnerException(e)})");
                        }
                    }
                }
            }
        ...
        public VideoInput SuggestVideo() {
            ...
            foreach (var plugin in plugins) {
                ...
                if (input != null) {
                    Log($"SuggestVideoInput {(input.InputData.Title != null ? input.InputData.Title : "Null Title")}");
                    return input;
                }
            }
        ...
        public AudioInput SuggestAudio() {
            ...
            foreach (var plugin in plugins) {
                ...
                if (input != null) {
                    Log($"SuggestAudioInput {(input.InputData.Title != null ? input.InputData.Title : "Null Title")}");
                    return input;
                }
            }
            return null;
        ...
        public void SearchSubtitles() {
            if (VideoInput != null && (((IOpen)VideoInput.Plugin).IsPlaylist || !m_searchedForSubtitles) && !VideoInput.SearchedForSubtitles) {
        ...
        private void Log(string msg) {
            Debug.WriteLine($"[{DateTime.Now.ToString("hh.mm.ss.fff")}] [#{UniqueId}] [PluginHandler] {msg}");
        }
    }
}