...
namespace FSPreview
{
    public unsafe static class Utils
    {
        ...
        public static int GetUniqueId() {
            Interlocked.Increment(ref m_uniqueId);
            return m_uniqueId;
        }
        ...
        /// </summary>
        /// <param name="ruleName">Default value is FsPlayer</param>
        ...
        public static string GetUrlExtention(string url) {
            return url.LastIndexOf(".") > 0 ? url.Substring(url.LastIndexOf(".") + 1) : "";
        }
        ...
        public static void EnsureThreadDone(Thread t, long maxMS = 15000, int minMS = 4) // Until Pause on live streams can be aborted must be > ReadLiveTimeout
        {
            ...
            if (t.ManagedThreadId == Thread.CurrentThread.ManagedThreadId) {
                Log($"Thread {t.Name} is not allowed to suicide!");
                return;
            }
        ...
        public static string GetUserDownloadPath() {
            try {
                return Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders\").GetValue("{374DE290-123F-4565-9164-39C4925E467B}").ToString();
            } catch (Exception) {
                return null;
            }
        }

        static List<PerformanceCounter> s_gpuCounters;

        public static void GetGPUCounters() {
            var category = new PerformanceCounterCategory("GPU Engine");
            var counterNames = category.GetInstanceNames();
            s_gpuCounters = new List<PerformanceCounter>();
            foreach (string counterName in counterNames) {
                if (counterName.EndsWith("engtype_3D")) {
                    foreach (PerformanceCounter counter in category.GetCounters(counterName)) {
                        if (counter.CounterName == "Utilization Percentage") {
                            s_gpuCounters.Add(counter);
                        }
                    }
                }
            }
        ...
        public static float GetGPUUsage() {
            float result = 0f;
            try {
                if (s_gpuCounters == null) GetGPUCounters();
                s_gpuCounters.ForEach(x => { _ = x.NextValue(); });
                Thread.Sleep(1000);
                s_gpuCounters.ForEach(x => { result += x.NextValue(); });
    ...
    public class SerializableDictionary<TKey, TVal> : Dictionary<TKey, TVal>, IXmlSerializable, ISerializable, INotifyPropertyChanged, INotifyCollectionChanged
    {
        ...
        protected XmlSerializer ValueSerializer {
            get { return m_valueSerializer ?? (m_valueSerializer = new XmlSerializer(typeof(TVal))); }
        }

        private XmlSerializer KeySerializer {
            get { return m_keySerializer ?? (m_keySerializer = new XmlSerializer(typeof(TKey))); }
        }
